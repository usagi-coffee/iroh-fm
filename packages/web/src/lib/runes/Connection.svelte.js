...
export class Connection {
	...
	loadLocalState() {
		...
		this.app.starredKey = localStorage.getItem('iroh-fm-starred-key') ?? localStorage.getItem('iroh-fm-loved-key') ?? '';
		const storedVolume = Number(localStorage.getItem('iroh-fm-volume'));
		if (Number.isFinite(storedVolume)) this.app.player.volume = Math.min(1, Math.max(0, storedVolume));
	}
	...
	async initializeIdentity() {
		...
		try {
			...
			if (generation === this.identityGeneration) this.error = friendlyError(error, 'Could not prepare a browser identity.');
		} finally {
	...
	async generateIdentity() {
		...
		try {
			...
			if (generation === this.identityGeneration) this.error = friendlyError(error, 'Could not generate a new client identity.');
		} finally {
			if (generation === this.identityGeneration) this.identityLoading = false;
		}
	}
	...
	async autoConnectOnce() {
		if (this.autoConnectAttempted || !this.ticket.trim() || !this.clientEndpointId) return;
	...
	/** @param {boolean} [forceTicket] @returns {Promise<boolean>} */
	async connect(forceTicket = false) {
		...
		try {
			...
			if (previousClient && previousClient !== nextClient) await previousClient.close().catch(() => {});
			return true;
		} catch (error) {
			await nextClient?.close().catch(() => {});
			if (operation !== this.operationGeneration) return false;
