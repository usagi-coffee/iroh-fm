...
export class Player {
	...
	async playAlbum(album) {
		const tracks = this.app.library.tracksForAlbum(album).filter((track) => !this.app.library.offlineOnly || track.cached);
	...
	prefetchNext(track, queue, generation) {
		...
		client.prefetchTrack(next.id, (received, total) => next.updateProgress(received, total, downloadGeneration))
			.then((cached) => {
	...
	async play(track, sourceQueue = this.app.library.tracks) {
		...
		if (previousTrack && !previousTrack.cached && previousDownloadGeneration !== null) previousTrack.stopDownload(previousDownloadGeneration);
		...
		try {
			...
			void source.done.then((cached) => {
				if (this.app.connection.client !== client) return;
				if (cached && this.app.library.tracksById.get(track.id) === track) this.app.library.markCached(track);
	...
	stop() {
		...
		this.audio?.pause();
		this.audioSource?.dispose();
		if (track && !track.cached && downloadGeneration !== null) track.stopDownload(downloadGeneration);
		this.audioSource = null;
		this.audioDownloadGeneration = null;
		this.audioSrc = '';
	...
	async toggle() {
		...
		if (this.audio.paused) await this.audio.play().catch((error) => (this.error = friendlyError(error, 'Playback was blocked.')));
	...
	onEnded() {
		if (this.repeat && this.audio) {
			...
			void this.audio.play().catch((error) => (this.error = friendlyError(error, 'Playback was blocked.')));
	...
	seek(value) {
		const audio = this.audio;
		const duration = Number.isFinite(audio?.duration) ? audio?.duration : this.currentTrack?.duration_seconds;
