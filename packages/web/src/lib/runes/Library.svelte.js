...
export class Library {
	...
	/** @type {Set<string>} */
	cachedTrackIds = $derived.by(() => new Set(this.tracks.filter((track) => track.cached).map((track) => track.id)));
	...
	/** @type {Track[]} */
	starredTracks = $derived.by(() => this.tracks.filter((track) => this.allStarredTrackIds.has(track.id)));
		...
		for (const [trackIndex, track] of filtered.entries()) {
			...
			if (albumKey !== previousAlbumKey) {
				...
				items.push({
					...
					durationSeconds: album?.duration_seconds ?? albumTracks.reduce((total, item) => total + (item.duration_seconds ?? 0), 0),
					tracks: albumTracks,
					album
				});
	...
	getVisibleAlbums() {
		return this.offlineOnly
			? this.albums.filter((album) => album.track_ids.length > 0 && album.track_ids.every((id) => this.cachedTrackIds.has(id)))
			: this.albums;
	}
	...
	async selectAlbum(album) {
		...
		const first = this.tracks.find((track) => ids.has(track.id) && (!this.offlineOnly || track.cached));
	...
	async activateAlbum(album) {
		...
		if (first && window.matchMedia('(max-width: 1023px)').matches) await this.app.player.playAlbum(album);
	}
	...
	async toggleStar(track, event) {
		...
		try {
			await client.request(this.app.starredKey
			...
			if (this.app.connection.client === client) this.app.connection.error = friendlyError(error, 'Could not update the starred track.');
		}
	}
	...
	async toggleOfflineOnly() {
		...
		if (this.offlineOnly && this.app.player.currentTrack && !this.app.player.currentTrack.cached) this.app.player.stop();
	}

	/** @param {Track} track */
	async cacheTrack(track) {
		if (this.offlineOnly || track.cached || track.downloading || this.cachingTrackIds.has(track.id)) return;
		const client = this.app.connection.client;
		if (!client) return;
		this.cachingTrackIds.add(track.id);
		const downloadGeneration = track.startDownload();
		try {
			const cached = await client.prefetchTrack(track.id, (received, total) => track.updateProgress(received, total, downloadGeneration));
			...
			if (this.app.connection.client === client) this.app.connection.error = friendlyError(error, 'Could not cache this track.');
		} finally {
	...
	async cacheAlbum(tracks, cacheKey) {
		...
		try {
			...
			if (this.app.connection.client === client) this.app.connection.error = friendlyError(error, 'Could not cache this album.');
		} finally {
	...
	async toggleStarAlbum(album) {
		...
		try {
			...
			if (this.app.connection.client === client) this.app.connection.error = friendlyError(error, 'Could not update the starred album.');
		}
	}
}
